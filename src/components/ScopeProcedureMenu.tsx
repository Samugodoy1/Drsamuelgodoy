...
export interface ScopeProcedureMenuProps {
  ...
  /** Próteses por intervalo: inicia a seleção de dentes no odontograma. */
  onStartRangeSelection: (proc: { procedureKey: string; procedure: string }) => void;
  hint?: string | null;
}
...
export const ScopeProcedureMenu: React.FC<ScopeProcedureMenuProps> = ({
  ...
  const pickRegionalProcedure = (procedureKey: string, procedure: string, target: 'quadrant' | 'arch') => {
    setPendingProc({ procedureKey, procedure });
    setView(target);
  };

  const startRange = (procedureKey: string, procedure: string) => {
    onStartRangeSelection({ procedureKey, procedure });
    close();
  };
          ...
          {view === 'list' && (
            <>
              ...
              {RANGE_SCOPE_PROCEDURES.map((proc) => (
                <button key={proc.key} type="button" onClick={() => startRange(proc.key, proc.label)} className={itemClass}>
                  {proc.label}
                  <span className="ml-1 text-[11px] font-normal text-slate-400">(clique nos dentes)</span>
                </button>
              ))}
