    ...
    <div className={compact ? '' : 'w-full'}>
      {!compact && (
        <div className="text-center mb-6 md:mb-12">
          <h2 className="apple-display-ink text-[26px] sm:text-[34px] md:text-[48px] leading-tight">{HUB_HEADLINE}</h2>
          {trialIncluded && (
            <p className="apple-subhead text-[15px] md:text-[21px] mt-2 md:mt-4 leading-snug">{HUB_FROM_LINE}</p>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5 items-stretch max-w-[820px] mx-auto">
        {HUB_PLANS.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`h-full rounded-[22px] md:rounded-[28px] p-5 sm:p-7 md:p-10 flex flex-col ${
                dark ? 'bg-[#1d1d1f] text-white' : 'bg-[#f5f5f7] text-[#1d1d1f]'
              } ${selected ? 'ring-2 ring-[#0071e3] ring-offset-2 ring-offset-white' : ''}`}
            >
              <h3 className="text-[18px] md:text-[24px] font-semibold tracking-tight">{plan.name}</h3>
              <p className={`mt-1.5 md:mt-2 text-[13px] md:text-[15px] ${dark ? 'text-white/50' : 'text-[#86868b]'}`}>{plan.line}</p>
              <p className="mt-4 md:mt-6 text-[28px] sm:text-[34px] md:text-[44px] font-semibold tracking-tight tabular-nums leading-none">
                R$&nbsp;{brl(amount)}
                <span className={`text-[14px] md:text-[17px] font-normal ${dark ? 'text-white/40' : 'text-[#86868b]'}`}>
                  {unit}
                </span>
              </p>
              <p className={`text-[12px] md:text-[13px] mt-2 mb-5 md:mb-8 leading-snug ${dark ? 'text-white/40' : 'text-[#86868b]'}`}>
                {trialIncluded
                  ? hubChargeAfterTrialLine(plan, cycle)
                  : cycle === 'yearly'
                    ? hubYearlyPerMonthLine(plan)
                    : 'Cobrada no início de cada mês.'}
              </p>
              <ul className={`space-y-2 md:space-y-3 text-[13px] md:text-[14px] flex-grow mb-5 md:mb-10 ${dark ? 'text-white/80' : 'text-[#1d1d1f]/80'}`}>
      ...
      <p className="mt-6 md:mt-10 text-center text-[11px] md:text-[12px] text-[#86868b] max-w-[520px] mx-auto leading-relaxed">
...
export function HubPlansScreen({
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f] px-4 py-8 sm:px-5 sm:py-12 md:py-24">
